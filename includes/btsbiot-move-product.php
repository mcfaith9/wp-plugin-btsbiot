...
// Mcfaith code for bulk copy Start
add_filter( 'bulk_actions-edit-post', 'mcfaith_my_bulk_multisite_actions' );

function mcfaith_my_bulk_multisite_actions( $bulk_array ) {
...
add_filter( 'handle_bulk_actions-edit-post', 'mcfaith_bulk_action_multisite_handler', 10, 3 );
...
function mcfaith_bulk_action_multisite_handler( $redirect, $doaction, $object_ids ) {
	...
	if( strpos( $doaction, "move_to_" ) === 0 ) {
		...
		$redirect = add_query_arg( array(
			'mcfaith_posts_moved' => count( $object_ids ),
...
// Notice
add_action( 'admin_notices', 'mcfaith_bulk_multisite_notices' );

function mcfaith_bulk_multisite_notices() {
	...
	if( ! empty( $_REQUEST['mcfaith_posts_moved'] ) ) {
		...
		printf( '<div id="message" class="updated notice is-dismissible"><p>' .
			_n( '%d post has been Moved/Copy to &quot;%s&quot;.', '%d posts have been Moved/Copy to &quot;%s&quot;.', intval( $_REQUEST['mcfaith_posts_moved'] )
