...
function wpmus_site_home_welcome_content() {
    ...
    <h3><?php _e( 'Welcome to Site "MCMultisite User Sync" Plugin', 'sd' ); ?></h3>
    <p>Thank you for choosing MCMultisite User Sync (which actually means "WordPress Multi-Site User Synchronization").</p>
...
function wpmus_site_home_concepts_content() {
    ...
    <p>MCMultisite User Sync has some simple but important concepts. Knowning all them will help you to get a better experience with the tool.</p>

    <h4>What exactly does this plugin do?</h4>
    <p>MCMultisite User Sync is a plugin that enable the user synchronization in your Wordpress Multisite, that is a type of WordPress installation that allows you to create and manage a network of multiple websites from a single WordPress dashboard.</p>
    <p>Key concepts:</p>
    <ul>
        <li>- MCMultisite User sync is a plugin, not a core feature of WordPress. It was built by external developers to WordPress. However, it goes through a detailed testing process to ensure smooth operation as it interacts with core aspects of the CMS.</li>
    ...
    <p>In MCMultisite User Sync you will can configure some triggers to automate user sync. A trigger is procedural code that is automatically executed in response to certain events, and in the particular case of MCMultisite User Sync & WordPress, to one of the following events:</p>
    <ul>
